...
Bootstrapping can take some time to run. Because no random seed was specified, exact values will vary slightly due to pseudorandom bootstrap sampling, but support values should be within about 1% of values reported in the main text. Manuscript figures can be generated directly from the saved ``.rdata`` files in ``/Bootstrap`` using ``/Code/03_plotting_X.R`` script. If one wants to recreate the manuscript numbers, please run ``/Code/04_tables_X.R`` script which used the saved ``.csv`` files in ``/Bootstrap``. 
...
3. ``02_Performance_Biomass_different-model-fit.R``: Prepare bootstrap data for Figure S4. 
4. ``02_Performance_density.R``: Generate Figure S2 and prepare bootstrap data for Figure S3 and Table S7-S8. 
5. ``03_plotting_Biomass.R``: Generate Figure 3-4. Prepare summary statistics in ``.csv`` for Tables S1-S6.
6. ``03_plotting_Biomass_different-model-fit.R``: Generate Figure S4.
7. ``03_plotting_Biomass.R``: Generate Figure S3. Prepare summary statistics in ``.csv`` for Tables S7-S8.
...
4. Other ``csv`` files are summary statistics of bootstrap samples for generating Table S1-S8.
